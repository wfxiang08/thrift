...
void t_go_generator::init_generator() {
    ...
    // Make output files
    f_types_name_ = package_dir_ + "/" + program_name_ + "-serializer.go";
...
void t_go_generator::generate_go_struct_writer(ofstream &out,
    ...
    vector<t_field *>::const_iterator f_iter;
    indent(out) << "func (p *" << tstruct_name << ") " << write_method_name_ << "(oprot thrift.TProtocol) error {" <<
    endl;
    indent_up();
...
void t_go_generator::generate_service_interface(t_service *tservice) {
    ...
    // interface定义导出到-model.go文件中
    f_types_model_ << indent() << "type " << interfaceName << " interface {" << extends_if;
    indent_up();
    generate_go_docstring(f_types_model_, tservice);
    vector<t_function *> functions = tservice->get_functions();
...
void t_go_generator::generate_service_client(t_service *tservice) {
    ...
    if (!extends_client.empty()) {
        ...
        f_types_model_ << indent() << "OutputProtocol thrift.TProtocol" << endl;
        f_types_model_ << indent() << "SeqId int32" << endl;
    ...
    f_types_model_ << indent() << "func New" << serviceName
    << "ClientFactory(t thrift.TTransport, f thrift.TProtocolFactory) *" << serviceName
    << "Client {" << endl;
    indent_up();
    f_types_model_ << indent() << "return &" << serviceName << "Client";

    if (!extends.empty()) {
        f_types_model_ << "{" << extends_field << ": " << extends_client_new << "Factory(t, f)}";
    } else {
        indent_up();
        f_types_model_ << "{Transport: t," << endl;
        f_types_model_ << indent() << "ProtocolFactory: f," << endl;
        f_types_model_ << indent() << "InputProtocol: f.GetProtocol(t)," << endl;
        f_types_model_ << indent() << "OutputProtocol: f.GetProtocol(t)," << endl;
        f_types_model_ << indent() << "SeqId: 0," << endl;
        /*f_types_ << indent() << "Reqs: make(map[int32]Deferred)" << endl*/;
        indent_down();
        f_types_model_ << indent() << "}" << endl;
    }
    ...
    if (!extends.empty()) {
        f_types_model_ << "{" << extends_field << ": " << extends_client_new << "Protocol(t, iprot, oprot)}"
        << endl;
    } else {
        indent_up();
        f_types_model_ << "{Transport: t," << endl;
        f_types_model_ << indent() << "ProtocolFactory: nil," << endl;
        f_types_model_ << indent() << "InputProtocol: iprot," << endl;
        f_types_model_ << indent() << "OutputProtocol: oprot," << endl;
        f_types_model_ << indent() << "SeqId: 0," << endl;
    ...
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        ...
        for (std::vector<t_field *>::size_type i = 0; i < num_args; ++i) {
            ...
            if (the_type2->is_enum()) {
                ...
                << "()" << endl;
                f_remote << indent() << err2 << " := argvalue" << i << "." << read_method_name_ << "(" << jsProt <<
                ")" << endl;
...
void t_go_generator::generate_process_function(t_service *tservice, t_function *tfunction) {
    ...
    if (!tfunction->is_oneway()) {
        ...
        f_types_ << indent() << "}" << endl;
        f_types_ << indent() << "if err2 = result." << write_method_name_ << "(oprot); err == nil && err2 != nil {" <<
        endl;
        f_types_ << indent() << "  err = err2" << endl;
        f_types_ << indent() << "}" << endl;
...
void t_go_generator::generate_serialize_container(ofstream &out,
    ...
    if (ttype->is_map()) {
        ...
        out << indent() <<
        "      return thrift.PrependError(\"\", fmt.Errorf(\"%T error writing set field: slice is not unique\", " <<
        prefix << "[i]))" << endl;
        out << indent() << "    }" << endl;
        out << indent() << "  }" << endl;
        out << indent() << "}" << endl;
